...
| `CONFIDENCE` | `0.65` | 检测置信度阈值 |
| `TRACK_TOLERANCE_MM` | `60.0` | 多目标时判定"还是同一个目标"的帧间跳变上限 (mm)，超过则重新判稳 |
...
| `POSE_REACH_TOLERANCE_M` | `0.02` | PTP 到位判定容差 (m)，超差视为未到位并回家 |
| `LATERAL_ERROR_MAX_M` | `0.05` | Point A 实际 Y/Z 相对果子的横向偏离上限 (m) |
| `IDLE_HOME_TIMEOUT_S` | `20.0` | 空闲且不在 hold-up 位多久后自动回家 (s)，0 关闭 |
...
| `NO TARGET` | 未检测到目标 |
| 横幅 `(3 seen, nearest selected)` | 视野里有 3 个目标，已选中最近的那个 |
| 灰框（细） | 看见了但不是最近的，只画不发布 |
| 框下 `settling 3/8` | 已累积 3 帧，需连续 8 帧 |
| 框下 `jitter 45mm > 15mm` | 散布超限，判为噪声 |
| 框下 `range 520mm (not nearest)` | 该目标的距离，未被选中 |
| 框下 `... | nearest of 3` | 选中框：它是 3 个目标里最近的 |
...
**多目标：只抓最近的那个**
视野里出现多个目标时，按相机坐标系的直线距离取最近的一个，其余只在预览里画成
灰框，不发布。近的果子既更好抓，也正好挡在伸向远处果子的路上。

判稳窗口跟着选中的目标走。相邻帧内选中点位移超过 `TRACK_TOLERANCE_MM`
（默认 60 mm）就判为换了目标：清空窗口、重新判稳。这条是必需的 ——
窗口发布的是 8 帧均值，混入另一个果子的坐标会算出一个两果之间的
虚假位置，臂会伸向空处。

早先的实现对**每个**检测框都往同一个窗口塞坐标，并且每个稳定框都发一次。
多果子场景下这有两个后果：窗口散布必然超限，永远判不稳；即使判稳，规划端
收到的也是循环里最后遍历到的那个框，与距离无关。
...
- 幅度是**相对当前角**而非绝对角。腕部实际落点每次都不同，按绝对角转会导致
  幅度时大时小
- 每轮结束回到起始角，避免朝一个方向越转越多顶到限位
- joint6 行程是 ±2π。若起始角离限位不足设定幅度，会自动降幅并打印告警；
  余量小于 1° 则跳过
- 拧转失败**不算抓取失败**：果子可能已经夹住，仍会继续退回。中途失败时会先
  尝试把腕部转回起始角，否则退回路径不可预测
- 拿不到 `/joint_states` 时跳过拧转并告警，不会盲目下发关节指令

调参：`TWIST_DEG=60 TWIST_CYCLES=3 ./start_peach_grasp.sh`，
不需要时 `TWIST_ENABLE=false`。
...
### 14.8.1 歪姿态死锁：为什么"已恢复"却一直不动

实测遇到过一种臂报 `err=0 state=2 mode=1`（完全健康）却再也动不了的状态。
自动恢复反复打印"✓ 已恢复"，臂却一步不走。链条是这样的：

1. **`move_ptp` 谎报成功。** 原实现只看 action 的 `status == 4`。但 C31 中途
   打断轨迹时，控制器停在半路，action 仍可能报完成。日志实证：指令 A 点
   `x=0.540, y≈0`，实际落到 `[0.538, 0.302, 0.508]`，**Y 偏 30 cm**，
   位置约束球半径只有 5 mm，却判成功。
2. **拿错位点当基准继续前进。** `current_pose_a` 取的是实际位姿，于是 B 点
   变成 `y=0.302`。运动前预检是在 STAGE 1 之前用**规划的** A 点做的，
   实际 A 偏掉之后没人再验一次，往这个方向推进直接顶出 C31。
3. **失败后不回家。** 所有中途失败路径都是裸 `return`，臂就停在坏姿态上，
   姿态偏差 28.6°。
4. **相机跟着歪，于是彻底锁死。** 相机装在末端，姿态一歪就照不到工作区。
   实测果子的相机坐标掉到 `z≈154 mm`，贴着 150 mm 深度下限，`/oak_result`
   停发。没有新目标 → 没有触发 → 没有运动 → 永远出不来。

**自动恢复只清控制器错误，不纠正几何位置**，这就是它一直说"已恢复"却没用的
原因。四处修复：

| 修复 | 作用 |
|---|---|
| `move_ptp` 增加 `_verify_arrived` | 用 TF 实测终点，偏差超 `POSE_REACH_TOLERANCE_M` 判失败，堵住谎报 |
| A 点横向偏离校验 | 实际 Y/Z 偏离果子超 `LATERAL_ERROR_MAX_M` 就不前进（前进只会抓空） |
| 失败路径统一 `return_home` | 任何动过之后的失败都回 hold-up，关节空间运动，失败再恢复重试一次 |
| 空闲看门狗 `_idle_home_check` | 兜底：不在 hold-up 位、无目标、空闲超 `IDLE_HOME_TIMEOUT_S` 就自动回家 |

看门狗与 `AUTO_EXECUTE` 无关，手动模式同样会卡死，而且那时操作员连可触发的
目标都没有。它在抓取中与冷却期不介入。

要停在原地做诊断就设 `IDLE_HOME_TIMEOUT_S=0`。
...
| 多个目标时反复 `TARGET SETTLING` | 两个目标距离接近，最近的那个在它们之间来回跳，每次跳变都重置窗口。把 `TRACK_TOLERANCE_MM` 调大，或把目标摆开一点 |
| 抓的不是最近那个 | 看预览里灰框标的 `range`：选中依据是相机直线距离，不是画面上的上下左右 |
...
| 反复"已恢复"但臂不动，`err=0` | 臂歪在半路，相机照不到工作区，没有目标可触发。看 14.8.1；新版会自动回家，旧版需手动 `recover` 或用 MoveIt 拖回 |
| `✗ 未到位：偏差 xxx mm` | PTP 谎报成功被拦下（通常是 C31 中断轨迹）。臂会自动回 hold-up 重来 |
| `Point A 横向偏离目标 xxx mm` | 规划器挑了个满足姿态但横向跑偏的解，前进会抓空，已拦下并回家 |
| `空闲 20s 且不在 hold-up 位` | 看门狗把臂救回来了，属正常保护。诊断时可设 `IDLE_HOME_TIMEOUT_S=0` |
...
| 多目标时永远判不稳 | 每个检测框都往同一个判稳窗口塞坐标，窗口里混着不同物体，散布必然超限 | 先收集全部候选，选最近的一个，只对它判稳 |
| 多目标时抓的目标不确定 | 每个稳定框都 publish 一次，规划端用到的是循环最后那个 | 一帧只发一条，固定为最近的目标 |
| 换目标瞬间可能发出两果之间的虚假坐标 | 判稳窗口发的是 8 帧均值，切换时新旧坐标被平均 | 位移超 `TRACK_TOLERANCE_MM` 判为换目标，清空窗口重新判稳 |
| 臂健康(`err=0`)却永久卡住，反复"已恢复" | `move_ptp` 只看 action 状态，C31 中断后谎报成功；臂歪在半路且失败路径不回家；相机随之照不到工作区，没有目标能再触发运动 | TF 实测到位校验 + A 点横向偏离校验 + 失败统一回家 + 空闲看门狗兜底（见 14.8.1） |
